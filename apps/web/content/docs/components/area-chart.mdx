...
| `formatXLabel` | `(date: Date) => string` | — | Formats axis ticks, the hover ticker, and the default tooltip title. Use a memoized `Intl.DateTimeFormat(...).format` for locale/timezone control. |
...
## Localized and hourly x labels

Pass `formatXLabel` to the chart root to keep the axis, hover ticker, and default
tooltip title in the same format. It also applies to domain ticks and projection
tails. The callback receives a `Date`, including generated ticks that have no
data row. Formatting changes labels only; it does not change the scale, brush
range, or the instants in your data.

```tsx
const formatHour = new Intl.DateTimeFormat("en-GB", {
  hour: "2-digit",
  minute: "2-digit",
  hourCycle: "h23",
  timeZone: "UTC",
...
The same prop is available on `LineChart` and `ComposedChart`. Create the formatter
outside rendering, or memoize it by locale and timezone when those can change.
Omitting the callback preserves the existing English short-date axis/ticker and
weekday-date tooltip title. Custom tooltip content remains yours to format.

Try the **XAxis → Labels → Format** presets in [Studio](/studio?chart=area-chart),
or the hourly example in the [Area Chart gallery](/charts/area-chart).
