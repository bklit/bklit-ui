...
### Series markers and dashed tail

`Area` supports the same `showMarkers`, `markers`, `dashFromIndex`, and `dashArray` props as `Line`. The dashed stroke follows the full curved path and respects the stroke gradient fade.

```tsx
<AreaChart data={weeklyData} xDataKey="date">
  <Grid horizontal />
  <Area
    dataKey="visitors"
    dashFromIndex={5}
