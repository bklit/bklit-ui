...
Orchestrates a main chart and optional brush strip. Manages brush selection state and passes `xDomain` / `xDomainSlotCount` to the main chart.
...
| `brushStrip` | `(layout) => ReactNode` | — | Mini chart + `ChartBrush` rendered below the main chart |
| `children` | `(layout) => ReactNode` | required | Main chart render function |

`layout` provides `xDomain`, `xDomainSlotCount`, `brushSelection`, and `onBrushSelectionChange`.

### ChartBrush

Renders the brush overlay inside a time-series chart (typically the brush strip). Drag handles to pan and resize the selection; the main chart zooms via `ChartBrushLayout`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `onSelectionChange` | `(domain) => void` | — | Fires on drag with the selected date range |
| `initialSelection` | `{ start: Date; end: Date }` | — | Initial brush window |
| `blurPx` | `number` | `1.5` | Backdrop blur on dimmed track regions |
| `fadeOuterEdges` | `boolean` | `true` | Fade dimmed regions at the outer track edges |
| `selectionPattern` | `{ preset; color }` | — | Optional pattern fill inside the selection window |
