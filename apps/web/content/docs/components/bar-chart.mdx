...
import { BarChartStackedDemo, BarChartHorizontalDemo, BarChart90DaysDemo, BarChartGradientDemo, BarChartPatternDemo } from "@/components/docs/bar-chart-demo";
...
<InstallationTabs name="bar-chart" dependencies={["@visx/gradient", "@visx/pattern", "@visx/shape", "motion"]} />
...
<ComponentShowcase code={`<BarChart data={data} xDataKey="month" stacked stackGap={3}>
  <Grid horizontal />
...
<ComponentShowcase code={`<BarChart data={data} xDataKey="browser" orientation="horizontal" margin={{ left: 80 }}>
...
<ComponentShowcase code={`<BarChart data={dailyData} xDataKey="day" barGap={0.1}>
  <Grid horizontal />
