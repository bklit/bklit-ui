...
import { SeriesStrokeDocsDemo } from "@/components/docs/series-stroke-docs-demo";
import {
...
Add scatter-style ring markers at each data point with `showMarkers`. Marker styling uses the same options as [`Scatter`](/docs/components/scatter-chart) via the `markers` prop:
...
Use `dashFromIndex` to draw a solid stroke through one point, then a dashed segment through the end of the series. Useful when the final period is still in progress (e.g. yesterday → today). The dashed segment follows the same curve and `fadeEdges` gradient as the solid stroke.

```tsx
<LineChart data={weeklyData} xDataKey="date">
  <Line
    dataKey="visitors"
    dashFromIndex={5}
    dashArray="6,4"
...
| `showMarkers` | `boolean` | `false` | Render ring markers at each point |
| `markers` | `SeriesPointMarkerStyle` | — | Marker radius, ring gap, ring width, hover fade |
| `dashFromIndex` | `number` | — | Data index where the dashed tail begins (inclusive) |
