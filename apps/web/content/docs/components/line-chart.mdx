...
### Unified `status` API (recommended)
...
**Choreography:** skeleton grid shimmer and line pulse loop → pulse exit → y-domain morph (500ms) → clip-path reveal (`cubic-bezier(0.85, 0, 0.15, 1)`) → interaction enabled.

Pair `Grid` shimmer props with `Line` `loadingStroke` props. Use `loadingLabel` on `LineChart` for centered shimmer text via [`@ncdai/shimmering-text`](https://chanhdai.com/components/shimmering-text).
...
### Standalone `LineChartLoading`

Use `LineChartLoading` for a loading-only placeholder without real chart data — dashed grid, optional shimmer, looping pulse, and label.
