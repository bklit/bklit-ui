...
Use `LineChartLoading` while data is fetching. It renders dashed grid lines with an optional sweeping shimmer, a looping foreground pulse (grow then exit), and a centered shimmer label via [`@ncdai/shimmering-text`](https://chanhdai.com/components/shimmering-text).

Grid and shimmer colors accept `color-mix` or `oklch` alpha strings so color and opacity are set together. Set `gridShimmerSync` to align the shimmer loop with the line pulse (2.2s cycle plus a 280ms pause between loops).
...
### Status and Y-domain tween (preview)

Toggle **Loading** / **Ready** on a single `LineChart` with one `Grid` and one `Line` — the loading pulse is driven by chart phase on `Line` (no separate `LineLoadingPulse` sibling).

<ComponentShowcase
  code={`const [status, setStatus] = useState<"loading" | "ready">("loading");

<LineChart data={data} status={status} yDomainTween>
  <Grid horizontal shimmer shimmerSync />
  <Line
    dataKey="revenue"
    loadingStroke="var(--foreground)"
    loadingStrokeOpacity={0.5}
    stroke={status === "ready" ? "var(--chart-line-primary)" : "transparent"}
  />
</LineChart>`}
...
| `gridStroke` | `string` | `color-mix(…)` on `--chart-grid` at 50% | Grid line color and opacity |
| `gridShimmerStroke` | `string` | `color-mix(…)` on `--foreground` at 68% | Shimmer band color and opacity |
| `gridShimmer` | `boolean` | `true` | Animate a shimmer band across grid lines |
| `gridShimmerLength` | `number` | `140` | Shimmer band width in pixels |
| `gridShimmerSpeed` | `number` | `1` | Shimmer speed multiplier when sync is off (higher = faster) |
| `gridShimmerSync` | `boolean` | `false` | Match shimmer timing to the line pulse (2.2s + 280ms pause) |
