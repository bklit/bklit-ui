...
`ComposedChart` accepts `formatXLabel?: (date: Date) => string`. It formats axis ticks,
the hover ticker and the default tooltip title, including generated domain and
projection ticks. See [localized and hourly labels](/docs/components/area-chart#localized-and-hourly-x-labels).
