...
- **`Line`** / **`Area`** — use **`showMarkers`** for scatter-style ring markers at each point; pass **`markers={{ radius, ringGap, strokeWidth }}`** to tune styling. Use **`dashFromIndex`** and **`dashArray`** for a dashed tail on incomplete periods (the dashed segment follows the same curve and edge fade).
...
### Markers and dashed tail on lines

Apply `showMarkers`, `dashFromIndex`, and `dashArray` on `Line` and `Area` inside a composed chart — useful when the final period is still in progress:

```tsx
<ComposedChart data={chartData} xDataKey="date">
  <Grid horizontal />
