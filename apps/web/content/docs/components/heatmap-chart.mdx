...
When using `loadingLabel` or `HeatmapChartLoading`, centered shimmer text uses [`@ncdai/shimmering-text`](https://chanhdai.com/components/shimmering-text) — installed automatically with `@bklit/heatmap-chart` when `@ncdai` is in your [registry config](/docs/installation#registry-namespace).
