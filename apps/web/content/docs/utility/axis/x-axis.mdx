...
For time-series charts, set `formatXLabel` on the parent `LineChart`, `AreaChart`,
or `ComposedChart` to format the axis, hover ticker, and default tooltip title
together. See [localized and hourly labels](/docs/components/area-chart#localized-and-hourly-x-labels).
