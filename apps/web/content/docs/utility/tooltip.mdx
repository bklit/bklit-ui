...
| `damping` | `number` | `20` | Spring damping for the floating tooltip panel follow. `0` = instant (no spring) |
...
- **Tooltip panel** - The floating content box eases behind the crosshair; use `damping` (`0`–`100`, default `20`) or `boxSpringConfig` to tune lag. Set `damping={0}` for instant panel follow with no easing delay.
