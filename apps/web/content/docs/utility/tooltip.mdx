...
import { TooltipDefaultDemo, TooltipCrosshairOnlyDemo, TooltipMinimalDemo, TooltipCustomRowsDemo, TooltipBarChartDemo, TooltipCustomContentDemo, TooltipSolidCrosshairDemo, TooltipCrosshairFadeTopDemo, TooltipDashedCrosshairDemo, TooltipPanelDampingDemo } from "@/components/docs/tooltip-demo";
...
| `matchCrosshair` | `boolean` | `true` | When `true`, the panel uses the crosshair spring and moves in sync |
| `damping` | `number` | `20` | Panel follow damping when `matchCrosshair={false}`. `0` = instant |
...
By default the tooltip panel shares the crosshair spring (`matchCrosshair`). Set `matchCrosshair={false}` and tune `damping` (`0` = instant, higher = more lag).
...
- **Tooltip panel** - By default (`matchCrosshair`) the panel shares the crosshair spring. Set `matchCrosshair={false}` to decouple panel follow, then tune lag with `damping` (`0`–`100`, default `20`) or `boxSpringConfig`.
