...
| `damping` | `number` | `30` | Spring damping for crosshair, dots, and date pill follow. `0` = instant (no spring) |
| `springConfig` | `{ stiffness: number; damping: number }` | chart config | Full spring override for crosshair, dots, and date pill |
| `boxSpringConfig` | `{ stiffness: number; damping: number }` | chart config | Spring override for the floating tooltip panel |
...
- **Crosshair** - Spring physics for snappy following; use `damping` (`0`–`100`, default `30`) or `springConfig` to tune lag. Set `damping={0}` for instant follow with no easing delay.
