...
import { DocsScrollArea } from "@/components/docs-scroll-area";
...
const linkStyles = {
  ...
  heading:
    "ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 font-medium text-muted-foreground text-xs outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
};

const studioNavLinks = [
  { text: "Overview", url: "/docs", external: false },
  { text: "Open Studio", url: "http://localhost:3000/studio", external: true },
];
      ...
      <DocsScrollArea className="h-full py-4 pt-9" showEdgeGradient>
        <nav className="p-10">
          <ul className="m-0 mb-4 list-none space-y-0.5 p-0">
            <li className={linkStyles.heading}>Studio</li>
            {studioNavLinks.map((link) => {
              const isActive = !link.external && pathname === link.url;
              return (
                <li key={link.url}>
                  <Link
                    className={cn(
